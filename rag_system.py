import json
import requests
from typing import Dict, Any, List, Optional
import logging
from openai import OpenAI
from pydantic import BaseModel, Field
from neo4j import GraphDatabase
from sentence_transformers import SentenceTransformer
import numpy as np

# Setup logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


# Define the structured output format
class ProductEntity(BaseModel):
    name: Optional[str] = None
    features: List[str] = Field(default_factory=list)
    category: Optional[str] = None


class DocumentEntity(BaseModel):
    type: Optional[str] = None
    name: Optional[str] = None


class RelationshipEntity(BaseModel):
    type: Optional[str] = None
    direction: Optional[str] = None


class QueryEntities(BaseModel):
    product: ProductEntity = Field(default_factory=ProductEntity)
    document: DocumentEntity = Field(default_factory=DocumentEntity)
    relationship: RelationshipEntity = Field(default_factory=RelationshipEntity)


class QueryParser:
    def __init__(self):
        """
        Initialize the query parser using OpenAI.
        """
        self.client = OpenAI(
            base_url="http://localhost:11434/v1",
            api_key="gemma3:1b",  # or your actual API key if using OpenAI
        )

    def parse_query(self, query: str) -> Dict[str, Any]:
        """
        Analyze the user query and extract relevant entities.

        Args:
            query: The user's query

        Returns:
            A dictionary containing the extracted entities
        """
        try:
            completion = self.client.beta.chat.completions.parse(
                model="gemma3:1b",  # or your preferred model
                messages=[
                    {
                        "role": "system",
                        "content": """
                        You are a query parser for a product knowledge graph.
                        Your task is to extract relevant entities from user queries.
                        
                        Extract the following entities:
                        1. Product:
                           - name: the main product name
                           - features: list of features (color, size, etc.)
                           - category: the product category
                        
                        2. Document:
                           - type: the type of document requested
                           - name: the specific document name
                        
                        3. Relationship:
                           - type: the type of relationship requested
                           - direction: the direction of the relationship
                        
                        If an entity is not present in the query, use null as value.
                        Use English for all extracted values.
                        """,
                    },
                    {"role": "user", "content": query},
                ],
                response_format=QueryEntities,
            )

            # Get the response content and parse it
            result_str = completion.choices[0].message.content
            result_dict = json.loads(result_str)
            
            # Convert to our structured format using model_dump() instead of dict()
            return QueryEntities(**result_dict).model_dump()

        except Exception as e:
            logger.error(f"Error parsing query: {e}")
            # Return empty dictionary in case of error
            return {
                "product": {"name": None, "features": [], "category": None},
                "document": {"type": None, "name": None},
                "relationship": {"type": None, "direction": None},
            }


class GraphSearcher:
    def __init__(self, uri: str, user: str, password: str):
        """
        Initialize the graph searcher.
        
        Args:
            uri: Neo4j connection URI
            user: Neo4j username
            password: Neo4j password
        """
        self.driver = GraphDatabase.driver(uri, auth=(user, password))
        self.model = SentenceTransformer('all-MiniLM-L6-v2')
        
    def close(self):
        """Close the Neo4j connection"""
        self.driver.close()
        
    def search(self, query: str, query_entities: Dict[str, Any], top_k: int = 5) -> Dict[str, Any]:
        """
        Search the graph using semantic similarity and graph relationships.
        
        Args:
            query: The original user query
            query_entities: The parsed query entities
            top_k: Number of results to return
            
        Returns:
            Dictionary containing search results
        """
        # Generate embedding for the query
        query_embedding = self.model.encode(query)
        
        with self.driver.session() as session:
            results = {
                "products": [],
                "documents": [],
                "annotations": []
            }
            
            # 1. Search for similar products
            if query_entities["product"]["name"]:
                product_results = session.run("""
                    MATCH (p:Product)
                    WHERE p.embedding IS NOT NULL
                    WITH p, 
                         reduce(acc = 0.0, x IN range(0, size(p.embedding)-1) |
                            acc + p.embedding[x] * $embedding[x]
                         ) / (
                            sqrt(reduce(acc = 0.0, x IN p.embedding | acc + x * x)) *
                            sqrt(reduce(acc = 0.0, x IN $embedding | acc + x * x))
                         ) AS similarity
                    ORDER BY similarity DESC
                    LIMIT $top_k
                    RETURN p.product_id as id,
                           p.name as name,
                           p.category_name as category,
                           similarity
                """, embedding=query_embedding.tolist(), top_k=top_k)
                
                results["products"] = [dict(record) for record in product_results]
            
            # 2. Search for similar documents
            if query_entities["document"]["type"]:
                doc_results = session.run("""
                    MATCH (d:Document)
                    WHERE d.embedding IS NOT NULL
                    WITH d, 
                         reduce(acc = 0.0, x IN range(0, size(d.embedding)-1) |
                            acc + d.embedding[x] * $embedding[x]
                         ) / (
                            sqrt(reduce(acc = 0.0, x IN d.embedding | acc + x * x)) *
                            sqrt(reduce(acc = 0.0, x IN $embedding | acc + x * x))
                         ) AS similarity
                    ORDER BY similarity DESC
                    LIMIT $top_k
                    RETURN d.filename as filename,
                           d.document_name as name,
                           similarity
                """, embedding=query_embedding.tolist(), top_k=top_k)
                
                results["documents"] = [dict(record) for record in doc_results]
            
            # 3. Search for similar annotations
            if query_entities["relationship"]["type"]:
                ann_results = session.run("""
                    MATCH (a:Annotation)
                    WHERE a.embedding IS NOT NULL
                    WITH a, 
                         reduce(acc = 0.0, x IN range(0, size(a.embedding)-1) |
                            acc + a.embedding[x] * $embedding[x]
                         ) / (
                            sqrt(reduce(acc = 0.0, x IN a.embedding | acc + x * x)) *
                            sqrt(reduce(acc = 0.0, x IN $embedding | acc + x * x))
                         ) AS similarity
                    ORDER BY similarity DESC
                    LIMIT $top_k
                    RETURN a.filename as filename,
                           a.content as content,
                           similarity
                """, embedding=query_embedding.tolist(), top_k=top_k)
                
                results["annotations"] = [dict(record) for record in ann_results]
            
            return results


class RAGSystem:
    def __init__(self, neo4j_uri: str, neo4j_user: str, neo4j_password: str):
        """
        Initialize the RAG system.
        
        Args:
            neo4j_uri: Neo4j connection URI
            neo4j_user: Neo4j username
            neo4j_password: Neo4j password
        """
        self.query_parser = QueryParser()
        self.graph_searcher = GraphSearcher(neo4j_uri, neo4j_user, neo4j_password)
        
    def close(self):
        """Close all connections"""
        self.graph_searcher.close()
        
    def process_query(self, query: str) -> Dict[str, Any]:
        """
        Process a user query through the RAG system.
        
        Args:
            query: The user's query
            
        Returns:
            Dictionary containing search results
        """
        # 1. Parse the query
        query_entities = self.query_parser.parse_query(query)
        
        # 2. Search the graph
        results = self.graph_searcher.search(query, query_entities)
        
        return {
            "query_entities": query_entities,
            "results": results
        }


# Usage example
if __name__ == "__main__":
    # Neo4j connection details
    NEO4J_URI = "bolt://localhost:7687"
    NEO4J_USER = "neo4j"
    NEO4J_PASSWORD = "password"
    
    # Initialize RAG system
    rag = RAGSystem(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD)
    
    try:
        # Test with some queries
        test_queries = [
            "Find products similar to the black mountain bike size 58",
            #"Show me the mountain bike manual",
            #"What products are compatible with the mountain bike handlebar?",
        ]
        
        for query in test_queries:
            print(f"\nQuery: {query}")
            result = rag.process_query(query)
            print("Result:")
            print(json.dumps(result, indent=2, ensure_ascii=False))
            
    finally:
        rag.close()
